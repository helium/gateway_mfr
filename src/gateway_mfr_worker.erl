...
init([Crypto]) ->
    case Crypto of
        ...
        tpm -> erlfapi:fapi_Initialize(null),
               {ok, #state{crypto=tpm}}
    end.
...
handle_provision(State=#state{crypto=tpm}) ->
    case erlfapi:fapi_Provision("", null, "") of
        ok -> ok = handle_provision_miner_key(State),
            handle_miner_key(State);
            Else -> Else
    end.
...
handle_provision_miner_key(#state{crypto=tpm}) ->
    case gen_compact_key(?KEY_PATH) of
        ok -> ok;
        {error, ResponseCode} -> DecodedResponse = erlfapi:fapi_RCDecode(ResponseCode),
...
handle_miner_key(#state{crypto=tpm}) ->
    case erlfapi:fapi_GetPublicKeyECC(?KEY_PATH) of
        {ok, PubPoint} ->
...
gen_compact_key(Path, N) when N > 0 ->
    case erlfapi:fapi_CreateKey(Path, "noDa, sign, decrypt", "", "") of
        ok -> case erlfapi:fapi_GetPublicKeyECC(Path) of
                  {ok, PubPoint} ->
                      CompactKey = {#'ECPoint'{point=PubPoint}, {namedCurve, ?secp256r1}},
                      case ecc_compact:is_compact(CompactKey) of
                          {true, _} -> ok;
                          false -> erlfapi:fapi_Delete(Path), gen_compact_key(Path, N - 1)
                      end;
                  _Else -> _Else
              end;
        _Else -> _Else
